...
public class ${domain.name}Action extends BaseAction{
	...
	//搜索可用的字段
	private String[] fields = {${domain.baseTypeFields}};
	...
	//查询出的实例集
	private List<${domain.name}> models = new LinkedList<Application>();
	...
	public String queryByProp(){
		initQuery();
		setModels(userService.findApplicationsByProp(property, keyword, user));
		return SUCCESS;
	}
